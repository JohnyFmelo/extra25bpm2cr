  ...
  return <Dialog open={isOpen} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-hidden flex flex-col px-[9px]">
        ...
        <div className="space-y-4 flex-1 overflow-hidden">
          ...
          {selectedVolunteers.length > 0 && <div className="space-y-2">
              <label className="text-sm font-medium">Para adicionar ({selectedVolunteers.length})</label>
              <div className="flex flex-wrap gap-2 max-h-20 overflow-y-auto">
                {selectedVolunteers.map(name => <div key={name} className="flex items-center gap-1 bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs">
                    <span>{name}</span>
                    <button onClick={() => setSelectedVolunteers(prev => prev.filter(n => n !== name))} className="hover:bg-green-200 rounded-full p-0.5">
                      <X className="h-3 w-3" />
                    </button>
                  </div>)}
              </div>
          ...
          <div className="space-y-2 flex-1 overflow-hidden">
            ...
            <div className="border rounded-md max-h-60 overflow-y-auto">
              {filteredUsers.length === 0 ? <div className="p-4 text-center text-gray-500">
                ...
                return <div key={user.id} className={`flex items-center space-x-3 p-2 rounded-md hover:bg-gray-50 cursor-pointer ${!isAdmin && atLimit && !isCurrentlyInSlot ? "opacity-50" : ""} ${isCurrentlyInSlot ? "bg-blue-50 border border-blue-200" : ""}`} onClick={() => !(!isAdmin && atLimit && !isCurrentlyInSlot) && handleVolunteerToggle(fullName)}>
                        <Checkbox checked={isSelected || isCurrentlyInSlot} disabled={!isAdmin && atLimit && !isCurrentlyInSlot} onChange={() => handleVolunteerToggle(fullName)} />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            ...
                            {isVolunteer && <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full ml-2 flex-shrink-0">
                                Voluntário
                              </span>}
                          </div>
          ...
          {isAdmin && <div className="text-xs text-blue-600 bg-blue-50 p-2 rounded">
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            <Button onClick={handleAddVolunteers} disabled={isLoading || selectedVolunteers.length === 0}>
