...
export const generateHistoricoContent = async (doc, currentY, data) => {
    ...
    yPos = addSectionTitle(doc, yPos, "PROVIDÊNCIAS E ANEXOS", "4", 1, data);
    yPos = addSectionTitle(doc, yPos, "PROVIDÊNCIAS", "4.1", 2, data);
    yPos = addWrappedText(doc, yPos, data.providencias || "Não informado.", MARGIN_LEFT, 12, "normal", MAX_LINE_WIDTH, 'justify', data);
    yPos += 2;

    yPos = addSectionTitle(doc, yPos, "DOCUMENTOS ANEXOS", "4.2", 2, data);
    yPos = addWrappedText(doc, yPos, data.documentosAnexos || "Nenhum.", MARGIN_LEFT, 12, "normal", MAX_LINE_WIDTH, 'left', data);
    yPos += 2;

    yPos = addSectionTitle(doc, yPos, "DESCRIÇÃO DOS OBJETOS/DOCUMENTOS APREENDIDOS", "4.3", 2, data);
    ...
    // --- SEÇÃO 4.4: FOTOS E/OU VÍDEOS ---
    const hasPhotos = Array.isArray(data.objetosApreendidos) && data.objetosApreendidos.length > 0;
    const hasVideos = Array.isArray(data.videoLinks) && data.videoLinks.length > 0;
    ...
    yPos = addSectionTitle(doc, yPos, sectionTitle, "4.4", 2, data);
    ...
    if (!hasPhotos && !hasVideos) {
        ...
        if (hasPhotos) {
            ...
            for (let i = 0; i < data.objetosApreendidos.length; i++) {
                ...
                try {
                    // Verifica se a foto é uma URL ou base64 válida
                    if (typeof photo === 'string' && (photo.startsWith('data:image') || photo.startsWith('http'))) {
                        doc.addImage(photo, 'JPEG', xPos, yPos, photoWidth, photoHeight);
                        doc.setFontSize(8);
                        doc.text(`Foto ${i + 1}`, xPos, yPos + photoHeight + 5);
            ...
            yPos = xPos !== MARGIN_LEFT ? yPos + photoHeight + 10 : yPos; // Ajusta yPos após as fotos
        }
        ...
        if (hasVideos) {
            ...
            for (let i = 0; i < data.videoLinks.length; i++) {
                ...
                try {
                    // Verifica se o link é uma string válida
