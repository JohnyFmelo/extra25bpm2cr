...
const GuarnicaoTab: React.FC<GuarnicaoTabProps> = ({
  ...
  const handleRemove = (originalIndex: number) => {
    const itemToRemove = currentGuarnicaoList[originalIndex];
    onRemovePolicial(originalIndex);
    toast({ title: "Removido", description: `Componente ${itemToRemove?.nome || ''} removido da guarnição.` });
  };

  const handleToggleApoio = (originalIndex: number) => {
    onToggleApoioPolicial(originalIndex);
  };
  ...
  const handleSaveNewOfficer = async () => {
    ...
    if (somenteNumeros(cpf).length !== 11 || !validateCPF(cpf)) {
    ...
    try {
      const dataToSave = { rgpm: rgpmNumeros, nome: nome.toUpperCase(), graduacao, pai: pai.toUpperCase(), mae: mae.toUpperCase(), naturalidade: naturalidade.toUpperCase(), cpf: somenteNumeros(cpf), telefone: telefoneNumeros };
      const { error } = await supabase.from("police_officers").upsert(dataToSave, { onConflict: "rgpm" });
  ...
  const isSaveDisabled = useCallback((): boolean => {
    ...
    return telNums.length !== 10 && telNums.length !== 11;
  }, [newOfficerFormData]);

  const renderPolicialItem = (componente: ComponenteGuarnicao, originalIndex: number, isCondutor: boolean) => {
    return (
      <div key={`${componente.rg}-${originalIndex}`} className={`flex items-center justify-between p-3 ${isCondutor ? "bg-blue-50" : "bg-background"} border-t`}>
        <div className="flex flex-col flex-grow mr-2 truncate">
          <span className="text-sm font-medium truncate" title={`${componente.posto} ${componente.nome} (RGPM: ${componente.rg})`}>
            {isCondutor && <span className="font-bold text-blue-800">(Condutor) </span>}
        ...
        <div className="flex items-center flex-shrink-0 space-x-2">
          {!isCondutor && (
            <div className="flex items-center space-x-1">
              <Label htmlFor={`apoio-switch-${originalIndex}`} className="text-xs text-muted-foreground">Apoio</Label>
              <Switch
                id={`apoio-switch-${originalIndex}`}
                checked={!!componente.apoio}
                onCheckedChange={() => handleToggleApoio(originalIndex)}
                aria-label={`Marcar ${componente.nome} como apoio`}
              />
            </div>
          )}
          <Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 h-8 w-8" onClick={() => handleRemove(originalIndex)} aria-label={`Remover ${componente.nome}`}>
  ...
  const guarnicaoPrincipal = currentGuarnicaoList.map((p, i) => ({ ...p, originalIndex: i })).filter(p => p.originalIndex === 0 || !p.apoio);
  const guarnicaoApoio = currentGuarnicaoList.map((p, i) => ({ ...p, originalIndex: i })).filter(p => p.originalIndex !== 0 && p.apoio);
  ...
  return <Card>
    <CardHeader>
      <div className="flex justify-between items-center">
        ...
        <Button variant="outline" size="sm" onClick={openRegisterDialog} type="button"><UserPlus className="h-4 w-4 mr-2" /></Button>
      </div>
    </CardHeader>
    <CardContent className="space-y-4 px-[8px]">
      ...
      {currentGuarnicaoList.length === 0 ? (
        ...
        <>
          {guarnicaoPrincipal.length > 0 && (
            <div className="space-y-2">
              <Label>Guarnição Principal</Label>
              <div className="border rounded-md overflow-hidden">
                {guarnicaoPrincipal.map((componente) => renderPolicialItem(componente, componente.originalIndex, componente.originalIndex === 0))}
              </div>
            </div>
          )}
          ...
          {guarnicaoApoio.length > 0 && (
            <div className="space-y-2 mt-4">
              <Label>Guarnição de Apoio</Label>
              <div className="border rounded-md overflow-hidden">
                {guarnicaoApoio.map((componente) => renderPolicialItem(componente, componente.originalIndex, false))}
              </div>
            </div>
          )}
        </>
    ...
    <Dialog open={isRegisterDialogOpen} onOpenChange={setIsRegisterDialogOpen}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader><DialogTitle>Cadastrar ou Atualizar Policial</DialogTitle></DialogHeader>
        <div className="grid gap-4 max-h-[70vh] overflow-y-auto pr-3 px-[5px] py-0 my-0">
          ...
          <div><Label htmlFor="dlg-nome">Nome Completo *</Label><Input id="dlg-nome" value={newOfficerFormData.nome} onChange={e => handleRegisterInputChange("nome", e.target.value)} placeholder="Nome completo" required /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><Label htmlFor="dlg-cpf">CPF *</Label><Input id="dlg-cpf" value={newOfficerFormData.cpf} onChange={e => handleRegisterInputChange("cpf", e.target.value)} placeholder="000.000.000-00" required inputMode="numeric" maxLength={14} /></div>
            <div><Label htmlFor="dlg-telefone">Telefone (com DDD) *</Label><Input id="dlg-telefone" value={newOfficerFormData.telefone} onChange={e => handleRegisterInputChange("telefone", e.target.value)} placeholder="(00) 00000-0000" required inputMode="tel" maxLength={15} /></div>
          </div>
          <div><Label htmlFor="dlg-naturalidade">Naturalidade (Cidade/UF) *</Label><Input id="dlg-naturalidade" value={newOfficerFormData.naturalidade} onChange={e => handleRegisterInputChange("naturalidade", e.target.value)} placeholder="Ex: Cuiabá/MT" required /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><Label htmlFor="dlg-pai">Nome do Pai *</Label><Input id="dlg-pai" value={newOfficerFormData.pai} onChange={e => handleRegisterInputChange("pai", e.target.value)} required placeholder="Nome completo do pai" /></div>
            <div><Label htmlFor="dlg-mae">Nome da Mãe *</Label><Input id="dlg-mae" value={newOfficerFormData.mae} onChange={e => handleRegisterInputChange("mae", e.target.value)} required placeholder="Nome completo da mãe" /></div>
          </div>
        </div>
        <DialogFooter>
