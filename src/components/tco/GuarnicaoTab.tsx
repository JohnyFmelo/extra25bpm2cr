                  ...
                  <div className="flex items-center flex-shrink-0 space-x-1"> {/* Container para botões de ação */}
                    {index > 0 && ( // Botão de Apoio apenas para não-condutores
                      <button
                        type="button"
                        role="switch"
                        aria-checked={componente.apoio}
                        onClick={() => handleToggleApoio(index)}
                        className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${
                          componente.apoio ? 'bg-blue-600' : 'bg-gray-300'
                        }`}
                        title={componente.apoio ? "Desmarcar como apoio" : "Marcar como apoio"}
                      >
                        <span className="sr-only">Usar como apoio</span>
                        <span
                          aria-hidden="true"
                          className={`inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${
                            componente.apoio ? 'translate-x-5' : 'translate-x-0'
                          }`}
                        />
                      </button>
                    )}
