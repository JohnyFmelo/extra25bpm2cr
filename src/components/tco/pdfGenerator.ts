...
export const generatePDF = async (inputData: any) => {
    ...
    try {
        ...
        // Por enquanto, apenas atualiza o documento no Firestore (remover quando migrar para Supabase)
        if (data.id) {
            ...
            console.log(`Documento TCO ${data.id} atualizado com URL temporária do PDF`);
        } else {
