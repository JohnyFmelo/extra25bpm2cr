...
// Função auxiliar para formatar a lista de policiais com "e" antes do último
const formatarGuarnicao = (componentes) => {
    if (!componentes || componentes.length === 0) return "[GUPM PENDENTE]";
    const nomesFormatados = componentes
        .filter(c => c.nome && c.posto) // Filtra componentes válidos
...
export const generateHistoricoContent = (doc, currentY, data) => {
    ...
    // Processa o relatoPolicial para substituir placeholders
    let relatoProcessado = data.relatoPolicial || "";
    const bairro = data.endereco ? data.endereco.split(',').pop()?.trim() || "[BAIRRO PENDENTE]" : "[BAIRRO PENDENTE]";
    relatoProcessado = relatoProcessado
        .replace("[VIATURA PENDENTE]", data.guarnicao || "[VIATURA PENDENTE]")
        .replace("[GUPM PENDENTE]", formatarGuarnicao(data.componentesGuarnicao))
        .replace("[BAIRRO PENDENTE]", bairro);

    yPos = addWrappedText(doc, yPos, relatoProcessado, MARGIN_LEFT, 12, "normal", MAX_LINE_WIDTH, 'justify', data);
    yPos += 2;
