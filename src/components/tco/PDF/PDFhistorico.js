...
export const generateHistoricoContent = async (doc, currentY, data) => {
    ...
    if (data.currentApoioList && data.currentApoioList.length > 0) {
        data.currentApoioList.forEach((componente, index) => {
            yPos = checkPageBreak(doc, yPos, 5, data);
            const text = `${index + 1}. ${componente.posto} ${componente.nome} (RGPM: ${componente.rg})`;
            doc.setFont("helvetica", "normal");
            doc.setFontSize(10);
            doc.text(text, MARGIN_LEFT + 5, yPos);
            yPos += 5;
        });
    } else {
        yPos = addWrappedText(doc, yPos, "Nenhum apoio informado.", MARGIN_LEFT, 12, "italic", MAX_LINE_WIDTH, 'left', data);
        yPos += 2;
    }
