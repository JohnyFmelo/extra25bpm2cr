...
type NewTimeSlotData = Omit<FirebaseTimeSlot, 'id' | 'volunteers' | 'slots_used'> & {
  slots_used?: number;
};
type EditTimeSlotData = Partial<FirebaseTimeSlot> & {
  id: string;
};
...
const TimeSlotDialog = ({
  ...
  const handleMilitaryTypeChange = (typeId: string, checked: boolean) => {
    setAllowedMilitaryTypesInternal(prev => checked ? [...prev, typeId] : prev.filter(type => type !== typeId));
  };
        ...
        <div className="space-y-5 py-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {slotOptions.map(slots => <Button key={slots} variant="outline" size="sm" className={cn("flex-1 min-w-10 h-10 border-gray-200", selectedSlots === slots && !showCustomSlots && "bg-green-500 text-white hover:bg-green-600 border-green-500")} onClick={() => {
              setSelectedSlots(slots);
              setShowCustomSlots(false);
            }} disabled={isLoading}>{slots}</Button>)}
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {militaryTypes.map(type => <div key={type.id} className="flex items-center space-x-2">
          ...
          {!editingTimeSlot && <div className="flex items-center justify-between gap-2 pt-2 pb-1">
