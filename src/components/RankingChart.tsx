...
const RankingList = () => {
  ...
  const getRankIcon = (position: number) => {
    switch (position) {
      ...
      default:
        return <div className="w-6 h-6 bg-slate-200 rounded-full flex items-center justify-center text-sm font-semibold text-slate-600">
            {position}
          </div>;
    }
  };
  ...
  return <Card className="shadow-lg w-full border-0 bg-gradient-to-br from-slate-50 to-white">
      <CardHeader className="pb-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg">
              {rankingType === "allowances" ? <DollarSign className="w-6 h-6 text-white" /> : <Calendar className="w-6 h-6 text-white" />}
            </div>
      ...
      <CardContent className="px-6 pb-6">
        {loading ? <div className="flex justify-center items-center py-12">
          ...
          </div> : data.length === 0 ? <div className="flex flex-col justify-center items-center py-12">
          ...
          return <div key={`${item.name}-${position}`} className={`
                  ...
                  <div className={`absolute inset-y-0 left-0 transition-all duration-700 ease-out
                      ${rankingType === "allowances" ? 'bg-gradient-to-r from-blue-100 to-blue-50' : 'bg-gradient-to-r from-green-100 to-green-50'}
                    `} style={{
              width: `${getProgressWidth(item.value, maxValue)}%`
            }} />
                  ...
                  <div className="relative flex items-center justify-between p-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      {isTopThree && <div className={`w-2 h-12 rounded-full
                          ${position === 1 ? 'bg-yellow-400' : position === 2 ? 'bg-gray-400' : 'bg-amber-500'}
                        `} />}
                    </div>
                  </div>
                </div>;
