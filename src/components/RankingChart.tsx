...
const RankingChart = () => {
  ...
  return <Card className="shadow-md w-full">
      <CardHeader>
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-transparent">Ranking</h2>
      ...
      maxHeight: '80vh'
        ...
        height: chartHeight
          ...
          </div> : data.length === 0 ? <div className="flex justify-center items-center" style={{
        height: chartHeight
      }}>
        ...
        height: chartHeight
            ...
            left: 10,
            bottom: 20
          }} // Aumentei margem direita para labels
          >
                <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} />
                <XAxis type="number" allowDecimals={rankingType === "allowances"} // Permite decimais para diárias
            />
                <YAxis type="category" dataKey="name" width={150} tick={{
              fontSize: 12,
              fill: '#333'
            }} interval={0} />
                <Tooltip formatter={(value: number, name, props) => {
              const formattedValue = rankingType === "allowances" ? value.toFixed(1) : value.toFixed(0);
              return [`${formattedValue} ${rankingType === "allowances" ? "diárias" : "viagens"}`, `Viajante: ${props.payload.name}`];
            }} cursor={{
              ...
              paddingTop: '10px'
                ...
                <Bar dataKey="value" fill={rankingType === "allowances" ? "#4f46e5" : "#10b981"} radius={[0, 4, 4, 0]}>
                  <LabelList dataKey="value" position="right" offset={5} style={{
                fill: '#333',
                fontSize: 12
