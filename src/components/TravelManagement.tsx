...
export const TravelManagement = () => {
  ...
  const handleArchive = async (travelId: string, newArchivedState: boolean) => {
    try {
      ...
      toast({
        title: "Erro",
        description: "Erro ao arquivar/desarquivar a viagem.",
        variant: "destructive"
      });
    }
  };
    ...
    try {
      ...
      if (travelData.isLocked) {
        toast({
          ...
          description: "Viagem com diárias processadas. Não é possível se candidatar/desistir.",
          variant: "destructive"
        });
        return;
      }
      ...
      toast({
        title: "Erro",
        description: "Erro ao se candidatar/desistir.",
        variant: "destructive"
      });
    }
  };
  const handleToggleLock = async (travelId: string) => {
    try {
      ...
      if (!isCurrentlyLocked) {
        ...
        toast({
          title: "Sucesso",
          description: "Viagem bloqueada e diárias processadas!"
        });
      } else {
        await updateDoc(travelRef, {
          isLocked: false,
  ...
  const getSortedVolunteers = (travel: Travel): {
    ...
    if (travel.isLocked) {
      ...
      displayList = displayList.map(v => ({
        ...v,
        isSelected: finalSelected.includes(v.fullName)
      }));
  ...
  const handleRemoveVolunteer = async (travelId: string, volunteerName: string) => {
    ...
    try {
      ...
      if (travelData.isLocked) {
        toast({
          title: "Ação não permitida",
          description: "Não é possível remover voluntários de uma viagem processada.",
          variant: "destructive"
        });
        return;
      }
  ...
  const handleManualVolunteerSelection = async (travelId: string, volunteerName: string, slots: number) => {
    ...
    if (!travel || travel.isLocked) {
      toast({
        title: "Ação não permitida",
        description: "Não se pode selecionar manualmente para viagens processadas.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (isAlreadySelected) {
      ...
      if (currentManualSelections.length < slots) {
        ...
        toast({
          ...
          description: `Máximo de ${slots} voluntários manuais atingido.`,
          variant: "destructive"
        });
      }
    }
  ...
  const handleOpenAddVolunteerDialog = (travelId: string) => {
    ...
    if (travel && travel.isLocked) {
      toast({
        title: "Ação não permitida",
        description: "Não é possível adicionar voluntários a viagens processadas.",
        variant: "destructive"
      });
      return;
    }
  ...
  return <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-transparent">
        ...
        return <Card key={travel.id} className={`relative overflow-hidden ${cardBg} border border-gray-200 shadow-md hover:shadow-lg transition-all duration-300 rounded-lg ${travel.archived ? "opacity-60 hover:opacity-100" : ""}`}>
                {statusBadge}
                ...
                {isAdmin && <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      ...
                      {today <= travelEnd && <DropdownMenuItem onClick={() => handleToggleLock(travel.id)} className="gap-2">
                ...
                <div className="p-4 cursor-pointer" onClick={() => toggleExpansion(travel.id)}>
                  ...
                  <div className="space-y-3">
                    ...
                    {(expandedTravels.includes(travel.id) || displayVolunteersList.length > 0 || isAdmin && !isLocked && today < travelStart) && <div className="pt-3 border-t border-gray-200">
                        <div className="flex justify-between items-center mb-2">
                          
                          {isAdmin && today < travelStart && !isLocked && <Button variant="outline" size="sm" className="h-7 px-2.5 text-xs" onClick={e => {
                    e.stopPropagation();
                    handleOpenAddVolunteerDialog(travel.id);
                        ...
                        {displayVolunteersList.length > 0 ? <div className="space-y-1.5 max-h-60 overflow-y-auto pr-1">
                            {displayVolunteersList.map(vol =>
                  // Usando a lista já processada
                  <div key={vol.fullName} title={isAdmin && !isLocked && today < travelStart ? "Duplo clique para selecionar/desselecionar manualmente" : ""} className={`text-sm p-2 rounded-md flex justify-between items-center transition-colors
                                            ${vol.isManual && !isLocked ? 'bg-yellow-50 border border-yellow-300 hover:bg-yellow-100' : vol.isSelected ? 'bg-green-100 border border-green-300 hover:bg-green-200' : 'bg-gray-100 border border-gray-200 hover:bg-gray-200'}
                    ...
                    if (isAdmin && !isLocked && today < travelStart) {
                      handleManualVolunteerSelection(travel.id, vol.fullName, travel.slots);
                              ...
                              <div className="flex items-center space-x-2 shrink-0">
                                ...
                                {isAdmin && today < travelStart && !isLocked && <Button variant="ghost" size="icon" className="h-6 w-6 hover:bg-red-100 rounded-full text-red-500" onClick={e => {
                        e.stopPropagation();
                        handleRemoveVolunteer(travel.id, vol.fullName);
                          ...
                          </div> : <p className="text-xs text-gray-500 italic">
                    ...
                    {canUserVolunteer && <div className="mt-4 pt-3 border-t border-gray-200">
                        <Button onClick={e => {
                  e.stopPropagation();
                  handleVolunteer(travel.id);
                }} className={`w-full shadow-sm transition-colors ${isUserVolunteered ? "bg-red-500 hover:bg-red-600" : "bg-blue-500 hover:bg-blue-600"} text-white font-medium`} disabled={travel.volunteers && travel.volunteers.length >= travel.slots && !isUserVolunteered} title={travel.volunteers && travel.volunteers.length >= travel.slots && !isUserVolunteered ? "Vagas esgotadas" : isUserVolunteered ? "Desistir da viagem" : "Quero ser voluntário"}>
      ...
      {isAdmin && <Button onClick={() => setIsModalOpen(true)} className="fixed bottom-6 right-6 rounded-full p-0 bg-blue-600 hover:bg-blue-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 h-14 w-14 flex items-center justify-center z-30" title="Criar Nova Viagem">
      ...
      <AddVolunteerDialog open={addVolunteerDialogOpen} onOpenChange={setAddVolunteerDialogOpen} travelId={selectedTravelId} currentVolunteers={travels.find(t => t.id === selectedTravelId)?.volunteers || []} onVolunteersAdded={() => {
      toast({
        title: "Sucesso",
