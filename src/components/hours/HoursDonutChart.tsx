...
export const HoursDonutChart = ({ 
  ...
  const data = [
    { name: 'Horas Trabalhadas', value: hours, color: chartColor },
    { name: 'Horas Restantes', value: remainingHours, color: '#E0E0E0' },
  ];

  // Get progress bar color based on the same thresholds
  const progressBarColor = hours < 13 ? 'bg-orange-500' : 
                          hours < 25 ? 'bg-yellow-300' : 
                          hours < 31 ? 'bg-blue-500' : 
                          hours < 39 ? 'bg-green-300' : 
                          'bg-green-500';
    ...
    <div className="flex flex-col items-center mt-4 space-y-2">
      <div className="h-40 w-full relative">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
          ...
          <div className="text-sm text-muted-foreground">de {maxHours}h</div>
        </div>
      </div>
