...
export const WorkedDaysCalendar = ({
  ...
  return <div className="rounded-xl border border-slate-200 p-4 space-y-4 py-0 my-0">
      ...
      <div className="grid grid-cols-7 gap-2 max-w-3xl mx-auto">
        {['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom'].map(day => <div key={day} className="text-center text-sm font-medium text-gray-500">
            {day}
          </div>)}
        ...
        return <div key={index} onClick={() => handleDayClick(date)} className={`
              ...
              {hours && <span className="font-medium">{hours}h</span>}
              {annotation && <span className={`text-xs ${location ? 'text-white' : 'text-gray-600'} truncate max-w-full px-1`}>
                  {annotation}
                </span>}
            </div>;
      })}
      ...
      <div className="flex flex-wrap gap-4 pt-2 border-t border-gray-200">
        {totals.bpm > 0 && <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-purple-600"></div>
            <span className="text-sm text-gray-600">25° BPM: {totals.bpm}h</span>
          </div>}
        {totals.saiop > 0 && <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-green-500"></div>
            <span className="text-sm text-gray-600">SAIOP: {totals.saiop}h</span>
          </div>}
        {totals.sinfra > 0 && <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-blue-500"></div>
            <span className="text-sm text-gray-600">SINFRA: {totals.sinfra}h</span>
          </div>}
      </div>

      <DateAnnotationDialog open={isAnnotationDialogOpen} onOpenChange={setIsAnnotationDialogOpen} date={selectedDate} onSave={handleSaveAnnotation} currentAnnotation={selectedDate ? annotations[format(selectedDate, 'yyyy-MM-dd')] : ''} />
    </div>;
};