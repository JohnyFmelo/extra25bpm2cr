...
import { Card, CardContent, CardDescription, CardHeader, CardTitle, CardFooter // Added if needed for specific sections
...
const TCOForm: React.FC<TCOFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (isDrugCase) {
      ...
      setVitimas([{
        ...initialPersonData,
        nome: "O ESTADO"
      }]); // Or clear it: setVitimas([{ ...initialPersonData }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const tcoDataParaPDF: any = {
        ...
        // Only include if there's a real victim narrative
        ...
        // Only include if real victim and representation chosen
  ...
  // Condutor for display derived from valid components
  ...
  return <div className="container px-4 py-6 md:py-10 max-w-5xl mx-auto">
      <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown} className="space-y-6" noValidate> {/* Adjusted spacing */}

        {/* Card: Basic Information */}
        <Card>
          <CardHeader>
            <CardTitle>Informações Básicas</CardTitle>
            <CardDescription className="">Identificação inicial do TCO e natureza principal.</CardDescription>
          </CardHeader>
          <CardContent>
            <BasicInformationTab tcoNumber={tcoNumber} setTcoNumber={setTcoNumber} natureza={natureza} setNatureza={setNatureza} autor={autor} setAutor={setAutor} // Still passed, though derived from autores list
        ...
        {/* Card: Drug Verification (Conditional) */}
        ...
        <Card>
          ...
          <CardContent>
            {/* Keep the existing grid layout for anexos inside the card content */}
            <div className="grid md:grid-cols-2 gap-6">
               ...
               <div className="p-6 border-2 border-dashed border-gray-300 rounded-lg flex flex-col space-y-4 hover:border-green-500 transition-colors duration-200 ease-in-out">
                 ...
                 <div className="flex w-full space-x-2 items-center pt-1">
                   {/* Using ShadCN Input component for consistency */}
                   <Input type="url" value={newVideoLink} onChange={e => setNewVideoLink(e.target.value)} placeholder="https://..." aria-label="Link do vídeo" className="flex-1 text-sm" // Adjusted class for ShadCN Input
                />
        ...
        {/* Submit Button Section (Kept separate for final action clarity) */}
        <div className="flex justify-end mt-8 pt-6 border-t border-gray-300"> {/* Added slightly more top margin */}
