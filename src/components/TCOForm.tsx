...
const TCOForm = () => {
  ...
  useEffect(() => {
    ...
    if (substancia === "Vegetal" && cor !== "Verde") {
      ...
      if (substancia === "Vegetal" && cor === "Verde") {
        setIndicios("Maconha");
      } else if (substancia === "Artificial" && cor === "Amarelada") {
        setIndicios("Pasta-Base");
      } // Corrigido nome popular
      else if (substancia === "Artificial" && cor === "Branca") {
        setIndicios("Cocaína");
      } else {
        setIndicios("");
      } // Limpa se não combinar
    }
  }, [substancia, cor]);
  useEffect(() => {
    /* ... set drug seizure description ... */
    if (natureza === "Porte de drogas para consumo" && indicios) {
      ...
      if (indicioFinal) {
        // Só preenche se tiver um indicio válido
  ...
  useEffect(() => {
    /* ... set tipificacao, pena, and conclusaoPolicial based on natureza ... */
  ...
  useEffect(() => {
    ...
    const gupm = componentesGuarnicao.filter(c => c.nome && c.posto).map(c => `${c.posto} ${c.nome}`) // Apenas Posto e Nome
    .join(", ") || "[GUPM PENDENTE]";
    ...
    updatedRelato = updatedRelato.replace("[HORÁRIO]", horaFato || "[HORÁRIO]").replace("[DATA]", dataFato ? new Date(dataFato + 'T00:00:00Z').toLocaleDateString('pt-BR') : "[DATA]") // Formata data
    .replace("[GUARNIÇÃO]", guarnicao || "[VIATURA PENDENTE]").replace("[OPERACAO_TEXT]", operacaoText).replace("[GUPM]", gupm).replace("[BAIRRO]", bairro).replace("[MEIO DE ACIONAMENTO]", comunicante || "[ACIONAMENTO]").replace("[NATUREZA]", displayNaturezaReal || "[NATUREZA PENDENTE]").replace("[LOCAL]", localFato || "[LOCAL PENDENTE]")
    // Deixar placeholders para edição manual
    .replace("[VERSÃO INICIAL]", "[DETALHAR VERSÃO INICIAL]").replace("[O QUE A PM DEPAROU]", "[DETALHAR O QUE A PM DEPAROU]").replace("[VERSÃO SUMÁRIA DAS PARTES E TESTEMUNHAS]", "[RESUMO DAS VERSÕES]").replace("[DILIGÊNCIAS E APREENSÕES REALIZADAS]", "[DESCREVER DILIGÊNCIAS/APREENSÕES]").replace("[ENCAMINHAMENTO PARA REGISTRO DOS FATOS]", "[DETALHAR ENCAMINHAMENTO]");
  ...
  useEffect(() => {
    /* ... sync autor principal com autores[0].nome ... */
  ...
  const handleRemoveVitima = (index: number) => {
    /* ...código original... */
    if (vitimas.length > 0) {
      // Permite remover mesmo se for o último
  ...
  const handleRemoveAutor = (index: number) => {
    ...
    if (autores.length > 0) {
      ...
      if (index === 0 && newAutores.length > 0) {
        setAutor(newAutores[0].nome);
      } else if (newAutores.length === 0) {
        setAutor("");
      } // Limpa se não houver mais autores
    }
  };
  ...
  const handleVitimaChange = (index: number, field: string, value: string) => {
    /* ...código original com formatCPF/Phone e validação... */
  ...
  const handleAutorDetalhadoChange = (index: number, field: string, value: string) => {
    /* ...código original com format/validate CPF/Phone e validação de idade... */
    const newAutores = [...autores];
    let processedValue = value;
    if (field === 'cpf') {
      processedValue = formatCPF(value);
  ...
  */
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (natureza === "Porte de drogas para consumo" && (!quantidade || !substancia || !cor || isUnknownMaterial && !customMaterialDesc)) {
      toast({
        ...
        description: "Preencha Quantidade, Substância, Cor e Descrição (se material desconhecido)."
      });
      return;
    }
    ...
    try {
      ...
      const tcoDataParaSalvar: any = {
        // Usar 'any' temporariamente para facilitar a remoção de undefined
        // Basic Info
        tcoNumber: tcoNumber.trim(),
        natureza: displayNaturezaReal,
        originalNatureza: natureza,
        // Guarda a seleção original
        customNatureza: customNatureza.trim(),
        // Guarda o texto customizado
        ...
        // Pessoas
        autores,
        // Array completo de autores detalhados
        vitimas: vitimasFiltradas,
        // Array filtrado de vítimas
        testemunhas: testemunhasFiltradas,
        // Array filtrado de testemunhas
        ...
        // Renomeado (Indício vira Nome Comum)
      ...
      toast({
        ...
        description: "Registrado com sucesso no banco de dados!"
      });
  ...
  // Mantido igual
  "Ameaça", "Vias de Fato", "Lesão Corporal", "Dano", "Injúria", "Difamação", "Calúnia", "Perturbação do Sossego", "Porte de drogas para consumo", "Outros"];
