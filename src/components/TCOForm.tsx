...
const TCOForm: React.FC<TCOFormProps> = ({ selectedTco, onClear }) => {
  ...
  const [hasMinorAuthor, setHasMinorAuthor] = useState<{ isMinor: boolean; details?: { years: number; months: number; days: number; index: number; }; }>({ isMinor: false });
  ...
  useEffect(() => {
    ...
    if (natureza === "Outros") {
      ...
      switch (natureza) {
        ...
        case "Conduzir veículo sem CNH gerando perigo de dano": tipificacaoAtual = "ART. 309 DO CÓDIGO DE TRÂNSITO BRASILEIRO"; penaAtual = "DETENÇÃO DE 6 MESES A 1 ANO, OU MULTA"; break;
        case "Entregar veículo automotor a pessoa não habilitada": tipificacaoAtual = "ART. 310 DO CÓDIGO DE TRÂNSITO BRASILEIRO"; penaAtual = "DETENÇÃO DE 6 MESES A 1 ANO, OU MULTA"; break;
        case "Trafegar em velocidade incompatível com segurança": tipificacaoAtual = "ART. 311 DO CÓDIGO DE TRÂNSITO BRASILEIRO"; penaAtual = "DETENÇÃO DE 6 MESES A 1 ANO, OU MULTA"; break;
        case "Omissão de socorro": tipificacaoAtual = "ART. 135 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 1 A 6 MESES, OU MULTA"; break;
        case "Rixa": tipificacaoAtual = "ART. 137 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 15 DIAS A 2 MESES, OU MULTA"; break;
        case "Invasão de domicílio": tipificacaoAtual = "ART. 150 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 1 A 3 MESES, OU MULTA"; break;
        case "Fraude em comércio": tipificacaoAtual = "ART. 176 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 6 MESES A 2 ANOS, OU MULTA"; break;
        case "Ato obsceno": tipificacaoAtual = "ART. 233 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 3 MESES A 1 ANO, OU MULTA"; break;
        case "Falsa identidade": tipificacaoAtual = "ART. 307 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 3 MESES A 1 ANO, OU MULTA"; break;
        case "Resistência": tipificacaoAtual = "ART. 329 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 2 MESES A 2 ANOS"; break;
        case "Desobediência": tipificacaoAtual = "ART. 330 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 15 DIAS A 6 MESES, OU MULTA"; break;
        case "Desacato": tipificacaoAtual = "ART. 331 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 6 MESES A 2 ANOS, OU MULTA"; break;
        case "Exercício arbitrário das próprias razões": tipificacaoAtual = "ART. 345 DO CÓDIGO PENAL"; penaAtual = "DETENÇÃO DE 15 DIAS A 1 MÊS, OU MULTA"; break;
  ...
  useEffect(() => {
    const updatedAutores = autores.map(autor => {
      if (autor.nome.trim() !== "" && !autor.relato) {
        ...
        const pronome = sexo === "feminino" ? "A AUTORA" : "O AUTOR";
        return { ...autor, relato: `${pronome} DOS FATOS ABAIXO ASSINADO, JÁ QUALIFICADO NOS AUTOS, CIENTIFICADO DE SEUS DIREITOS CONSTITUCIONAIS INCLUSIVE O DE PERMANECER EM SILÊNCIO, DECLAROU QUE [INSIRA DECLARAÇÃO]. LIDO E ACHADO CONFORME. NADA MAIS DISSE E NEM LHE FOI PERGUNTADO.` };
      }
      return autor;
    });
    ...
    if (!alreadyExists) {
      setComponentesGuarnicao(prevList => {
        const newList = (prevList.length === 0 || (prevList.length === 1 && !prevList[0].rg && !prevList[0].nome && !prevList[0].posto))
          ? [{ ...novoPolicial, apoio: false }]
          : [...prevList, { ...novoPolicial, apoio: false }];
        return newList;
      });
      toast({ title: "Adicionado", description: `Policial ${novoPolicial.nome} adicionado à guarnição.`, className: "bg-green-600 text-white border-green-700", duration: 5000 });
    } else {
      toast({ title: "Duplicado", description: "Este policial já está na guarnição.", className: "bg-yellow-600 text-white border-yellow-700", duration: 5000 });
    }
  ...
  const handleToggleApoioPolicial = useCallback((index: number) => {
    setComponentesGuarnicao(prevList => {
      ...
      updatedList[index] = { ...updatedList[index], apoio: !updatedList[index].apoio };
      return updatedList;
    });
    toast({ title: "Status Alterado", description: `Policial ${componentesGuarnicao[index].nome} agora é ${componentesGuarnicao[index].apoio ? 'da guarnição principal' : 'de apoio'}.`, className: "bg-blue-600 text-white border-blue-700", duration: 5000 });
  }, [componentesGuarnicao, toast]);
  ...
  const handleRemoveAutor = (index: number) => {
    ...
    setHasMinorAuthor({ isMinor: !!minorAuthor });
  };
  ...
  const handleAutorDetalhadoChange = (index: number, field: string, value: string) => {
    ...
    if (field === 'cpf') {
      ...
      if (value && !isNaN(birthDate.getTime())) {
        ...
        const age = calculateAge(birthDate, today);
        if (age.years < 18) {
          setHasMinorAuthor({ isMinor: true, details: { years: age.years, months: age.months, days: age.days, index } });
          toast({ title: "Autor Menor de Idade", description: `O autor ${autores[index].nome || 'sem nome'} possui ${age.years} anos, ${age.months} meses e ${age.days} dias. TCO não pode ser registrado para menores de 18 anos.`, className: "bg-red-600 text-white border-red-700", duration: 10000 });
        } else {
          setHasMinorAuthor(prev => (prev.details?.index === index ? { isMinor: false } : prev));
        }
      } else {
        setHasMinorAuthor(prev => (prev.details?.index === index ? { isMinor: false } : prev));
      }
  ...
  const handleAddVideoLink = () => {
    if (newVideoLink.trim() && !videoLinks.includes(newVideoLink.trim())) {
      if (!/^(https?:\/\/)/i.test(newVideoLink.trim())) {
        toast({ title: "Link Inválido", description: "Por favor, insira um link válido começando com http:// ou https://.", className: "bg-yellow-600 text-white border-yellow-700", duration: 7000 });
        return;
      }
      setVideoLinks(prev => [...prev, newVideoLink.trim()]);
      setNewVideoLink("");
      toast({ title: "Link Adicionado", description: "Link de vídeo adicionado com sucesso.", className: "bg-green-600 text-white border-green-700", duration: 5000 });
    } else if (!newVideoLink.trim()) {
      toast({ title: "Link Vazio", description: "Por favor, insira um link.", className: "bg-yellow-600 text-white border-yellow-700", duration: 5000 });
    } else {
      toast({ title: "Link Duplicado", description: "Este link já foi adicionado.", className: "bg-yellow-600 text-white border-yellow-700", duration: 5000 });
    }
  };

  const handleRemoveVideoLink = (index: number) => {
    setVideoLinks(prev => prev.filter((_, i) => i !== index));
    toast({ title: "Link Removido", description: "Link de vídeo removido com sucesso.", className: "bg-green-600 text-white border-green-700", duration: 5000 });
  };
  ...
  const handleImageFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      if (uniqueNewFiles.length > 0) {
        ...
        toast({ title: `${uniqueNewFiles.length} Imagem(ns) Adicionada(s)`, description: "Imagens selecionadas para anexo.", className: "bg-green-600 text-white border-green-700", duration: 5000 });
      } else if (newFiles.length > 0) {
        toast({ title: "Imagens Duplicadas", description: "Algumas ou todas as imagens selecionadas já foram adicionadas.", className: "bg-yellow-600 text-white border-yellow-700", duration: 5000 });
      }
  ...
  const handleRemoveImageFile = (index: number) => {
    ...
    toast({ title: "Imagem Removida", description: "Imagem removida da lista de anexos.", className: "bg-green-600 text-white border-green-700", duration: 5000 });
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (hasMinorAuthor.isMinor) {
      toast({ title: "Submissão Bloqueada", description: `Não é possível registrar o TCO porque o autor ${autores[hasMinorAuthor.details!.index].nome || 'sem nome'} é menor de idade (${hasMinorAuthor.details!.years} anos, ${hasMinorAuthor.details!.months} meses, ${hasMinorAuthor.details!.days} dias).`, className: "bg-red-600 text-white border-red-700", duration: 10000 });
      return;
    }
    ...
    if (!tcoNumber.trim()) {
      toast({ title: "Campo Obrigatório", description: "O número do TCO é obrigatório.", className: "bg-red-600 text-white border-red-700", duration: 7000 });
      return;
    }
    if (natureza === "Selecione...") {
      toast({ title: "Campo Obrigatório", description: "Selecione a Natureza da Ocorrência.", className: "bg-red-600 text-white border-red-700", duration: 7000 });
      return;
    }
    if (natureza === "Outros" && !customNatureza.trim()) {
      toast({ title: "Campo Obrigatório", description: "Descreva a Natureza em 'Outros'.", className: "bg-red-600 text-white border-red-700", duration: 7000 });
      return;
    }
    if (autoresValidos.length === 0) {
      toast({ title: "Campo Obrigatório", description: "Adicione pelo menos um Autor com nome.", className: "bg-red-600 text-white border-red-700", duration: 7000 });
      return;
    }
    const componentesValidos = componentesGuarnicao.filter(c => c.nome?.trim() && c.rg?.trim());
    if (componentesValidos.length === 0) {
      toast({ title: "Campo Obrigatório", description: "Adicione pelo menos um Componente da Guarnição válido (Nome e RG).", className: "bg-red-600 text-white border-red-700", duration: 7000 });
      return;
    }
    if (natureza === "Porte de drogas para consumo" && (!quantidade.trim() || !substancia || !cor || (isUnknownMaterial && !customMaterialDesc.trim()) || !lacreNumero.trim())) {
      toast({ title: "Dados da Droga Incompletos", description: "Para Porte de Drogas, preencha Quantidade, Substância, Cor, Número do Lacre e Descrição (se material desconhecido).", className: "bg-red-600 text-white border-red-700", duration: 7000 });
      return;
    }
    ...
    try {
      ...
      for (const file of imageFiles) {
        try {
          ...
          toast({ title: "Erro ao Processar Imagem", description: `Não foi possível processar a imagem ${file.name}. Ela não será incluída.`, className: "bg-red-600 text-white border-red-700", duration: 7000 });
        }
      }
      ...
      const tcoMetadata = {
        tconumber: tcoNumber.trim(),
        natureza: displayNaturezaReal,
        policiais: componentesValidos.map(p => ({ nome: p.nome, rgpm: p.rg, posto: p.posto, apoio: !!p.apoio })),
        pdfpath: filePath,
        pdfurl: downloadURL,
        createdby: userId,
      ...
      toast({ title: "TCO Registrado com Sucesso!", description: "PDF enviado e informações salvas no sistema.", className: "bg-green-600 text-white border-green-700", duration: 5000 });
      navigate("/?tab=tco");
      ...
      toast({ title: "Erro ao Finalizar TCO", description: `Ocorreu um erro: ${errorMessage}`, className: "bg-red-600 text-white border-red-700", duration: 10000 });
    } finally {
  ...
  return (
    <div className="container md:py-10 max-w-5xl mx-auto py-0 px-[9px]">
      <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown} className="space-y-6" noValidate>
        ...
        {/* NEW SECTION FOR JUIZADO ESPECIAL */}
        <div className="mb-8 pb-8 border-b border-gray-200 last:border-b-0 last:pb-0">
          <h2 className="text-xl font-semibold mb-4">Apresentação em Juizado Especial</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="juizadoData" className="block text-sm font-medium text-gray-700 mb-1">
                Data da Audiência
              </label>
              <Input
                id="juizadoData"
                ...
                className="w-full"
                aria-label="Data da audiência no Juizado Especial"
              />
            </div>
            <div>
              <label htmlFor="juizadoHora" className="block text-sm font-medium text-gray-700 mb-1">
                Hora da Audiência
              </label>
              <Input
                id="juizadoHora"
                ...
                className="w-full"
                aria-label="Hora da audiência no Juizado Especial"
              />
            </div>
          </div>
          <p className="text-xs text-gray-500 mt-4">
            Informe a data e hora para comparecimento ao Juizado Especial Criminal. Estes campos são distintos da data e hora do fato.
          </p>
        </div>
        ...
        <div className="mb-8 pb-8 border-b border-gray-200 last:border-b-0 last:pb-0">
          ...
          <GeneralInformationTab
            natureza={natureza}
            tipificacao={tipificacao} setTipificacao={setTipificacao}
        ...
        <div className="mb-8 pb-8 border-b border-gray-200 last:border-b-0 last:pb-0">
          ...
          <PessoasEnvolvidasTab
            vitimas={vitimas} handleVitimaChange={handleVitimaChange}
            handleAddVitima={handleAddVitima} handleRemoveVitima={handleRemoveVitima}
            testemunhas={testemunhas} handleTestemunhaChange={handleTestemunhaChange}
            handleAddTestemunha={handleAddTestemunha} handleRemoveTestemunha={handleRemoveTestemunha}
            autores={autores} handleAutorDetalhadoChange={handleAutorDetalhadoChange}
            handleAddAutor={handleAddAutor} handleRemoveAutor={handleRemoveAutor}
            natureza={natureza}
          />
        </div>
        ...
        <div className="mb-8 pb-8 border-b border-gray-200 last:border-b-0 last:pb-0">
          ...
          <HistoricoTab
            ...
            natureza={natureza}
            videoLinks={videoLinks} setVideoLinks={setVideoLinks}
            solicitarCorpoDelito={autores.length > 0 ? autores[0].laudoPericial : "Não"}
            autorSexo={autores.length > 0 ? autores[0].sexo : "masculino"}
            providencias={providencias} setProvidencias={setProvidencias}
            documentosAnexos={documentosAnexos} setDocumentosAnexos={setDocumentosAnexos}
            lacreNumero={lacreNumero}
            vitimas={vitimas} setVitimaRelato={handleVitimaRelatoChange} setVitimaRepresentacao={handleVitimaRepresentacaoChange}
            testemunhas={testemunhas} setTestemunhaRelato={handleTestemunhaRelatoChange}
                    ...
                    <ul className="space-y-1.5 max-h-40 overflow-y-auto border border-gray-200 rounded-md p-2 bg-gray-50">
                      {videoLinks.map((link, index) => (
                        <li key={`${index}-${link}`} className="flex justify-between items-center p-1.5 bg-white border border-gray-200 rounded-md text-sm group shadow-sm">
                          <a href={link} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all line-clamp-2 mr-2 flex-1" title={`Abrir link: ${link}`}>
                            {link}
                          </a>
