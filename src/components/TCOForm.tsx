...
const TCOForm = () => {
  ...
  useEffect(() => {
    if (natureza === "Porte de drogas para consumo" && indicios) {
      ...
      if (indicioFinal) {
        ...
        const descriptiveText = isUnknownMaterial ? `${quantidadeText} ${plural} PEQUENA DE SUBSTÂNCIA DE MATERIAL DESCONHECIDO, ${customMaterialDesc || "[DESCRIÇÃO]"}, CONFORME FOTO EM ANEXO.` : `${quantidadeText} ${plural} PEQUENA DE SUBSTÂNCIA ANÁLOGA A ${indicioFinal.toUpperCase()}, ${customMaterialDesc || "[DESCRIÇÃO]"}, CONFORME FOTO EM ANEXO.`;
  ...
  useEffect(() => {
    ...
    const autorTexto = autores.length === 0 ? "O(A) AUTOR(A)" : autores.length === 1 ? autores[0].sexo.toLowerCase() === "feminino" ? "A AUTORA" : "O AUTOR" : autores.every(a => a.sexo.toLowerCase() === "feminino") ? "AS AUTORAS" : "OS AUTORES";
  ...
  useEffect(() => {
    ...
    console.log("Atualizando relatoPolicial com:", {
      horaFato,
      dataFato,
      guarnicao,
      operacao,
    ...
    updatedRelato = updatedRelato.replace("[HORÁRIO]", horaFato || "[HORÁRIO]").replace("[DATA]", dataFato ? new Date(dataFato + 'T00:00:00Z').toLocaleDateString('pt-BR') : "[DATA]").replace("[GUARNIÇÃO]", guarnicao || "[GUARNIÇÃO PENDENTE]").replace("[OPERACAO_TEXT]", operacaoText).replace("[GUPM]", gupm).replace("[BAIRRO]", bairro).replace("[MEIO DE ACIONAMENTO]", comunicante || "[ACIONAMENTO]").replace("[NATUREZA]", displayNaturezaReal || "[NATUREZA PENDENTE]").replace("[LOCAL]", localFato || "[LOCAL PENDENTE]").replace("[VERSÃO INICIAL]", "[DETALHAR VERSÃO INICIAL]").replace("[O QUE A PM DEPAROU]", "[DETALHAR O QUE A PM DEPAROU]").replace("[VERSÃO SUMÁRIA DAS PARTES E TESTEMUNHAS]", "[RESUMO DAS VERSÕES]").replace("[DILIGÊNCIAS E APREENSÕES REALIZADAS]", "[DESCREVER DILIGÊNCIAS/APREENSÕES]").replace("[ENCAMINHAMENTO PARA REGISTRO DOS FATOS]", "[DETALHAR ENCAMINHAMENTO]");
  ...
  const handleRemoveAutor = (index: number) => {
    if (autores.length > 0) {
      ...
      setAutores(newAutores);
      if (index === 0 && newAutores.length > 0) setAutor(newAutores[0].nome);else if (newAutores.length === 0) setAutor("");
    }
  };
  ...
  return <div className="container py-6 md:py-10 max-w-5xl mx-auto px-0">
      <form onSubmit={handleSubmit} className="space-y-6">
        <BasicInformationTab tcoNumber={tcoNumber} setTcoNumber={setTcoNumber} natureza={natureza} setNatureza={setNatureza} autor={autor} setAutor={setAutor} penaDescricao={penaDescricao} naturezaOptions={naturezaOptions} customNatureza={customNatureza} setCustomNatureza={setCustomNatureza} startTime={startTime} isTimerRunning={isTimerRunning} />
        {natureza === "Porte de drogas para consumo" && <DrugVerificationTab quantidade={quantidade} setQuantidade={setQuantidade} substancia={substancia} setSubstancia={setSubstancia} cor={cor} setCor={setCor} indicios={indicios} customMaterialDesc={customMaterialDesc} setCustomMaterialDesc={setCustomMaterialDesc} isUnknownMaterial={isUnknownMaterial} lacreNumero={lacreNumero} setLacreNumero={setLacreNumero} />}
        ...
        <PessoasEnvolvidasTab activeTab={activeTab} setActiveTab={setActiveTab} autorDetalhes={autores[0] || {
        nome: "",
        sexo: "",
        estadoCivil: "",
        profissao: "",
