...
// Interface para os dados de componentes da guarnição
...
// Interface para os dados de pessoas envolvidas (autores, vítimas, testemunhas)
interface Pessoa {
  nome: string;
  sexo: string;
  estadoCivil: string;
...
// Função para formatar a representação da vítima
...
// Função para formatar os componentes da guarnição
...
// Função para formatar o relato do autor
...
const TCOForm = () => {
  ...
  // Iniciar cronômetro quando o número do TCO é preenchido
  useEffect(() => {
  ...
  // Atualizar indícios de droga com base em substância e cor
  useEffect(() => {
  ...
  // Atualizar apreensões e limpar vítimas em casos de drogas
  useEffect(() => {
    if (natureza === "Porte de drogas para consumo" && indicios) {
      ...
      setLacreNumero("");
      // Não limpar vítimas aqui para manter a coluna visível
      // setVitimas([]);
      setRelatoVitima("");
      setRepresentacao("");
    }
  ...
  // Atualizar tipificação, pena e conclusão policial
  useEffect(() => {
  ...
  // Atualizar relato policial automaticamente
  useEffect(() => {
  ...
  // Função para adicionar policial à guarnição
  ...
  // Função para remover policial da guarnição
  ...
  // Funções para gerenciar testemunhas
  ...
  // Função para atualizar dados de vítimas
  ...
  // Função para atualizar dados de testemunhas
  ...
  // Função para atualizar dados de autores
  ...
  // Função para atualizar relato policial manualmente
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validações específicas para casos de drogas
    ...
    try {
      ...
      // Dados a serem salvos no Firebase
      ...
      // Incluir relato da vítima e representação apenas para casos não relacionados a drogas
  ...
  // Opções de natureza da ocorrência
  ...
  // Dados do condutor para exibição
  ...
  return (
    <div className="container px-4 py-6 md:py-10 max-w-5xl mx-auto">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        />
        {/* Aba de Verificação de Drogas (apenas para casos de drogas) */}
        ...
        <PessoasEnvolvidasTab
          ...
          natureza={natureza}
          // Adicionada prop para controlar a visibilidade da coluna de vítima
          mostrarColunaVitima={natureza !== "Porte de drogas para consumo"}
        />
        {/* Aba de Guarnição */}
        <GuarnicaoTab
