...
const AddVolunteerDialog = ({
  ...
  const filteredUsers = users.filter(user => {
    ...
    return warName.includes(searchLower) || rank.includes(searchLower) || email.includes(searchLower) || fullName.includes(searchLower);
  });
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="py-4">
          <div className="relative mb-4">
            <Input placeholder="Buscar por nome, graduação ou email..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-8" />
          ...
          {loading ? <div className="flex justify-center items-center py-8">
              ...
              {filteredUsers.length > 0 ? <div className="space-y-2">
                  {filteredUsers.map(user => {
              const isAlreadyAdded = isVolunteerAlreadyAdded(user.id);
              const isSelected = selectedUsers.includes(user.id);
              return <div key={user.id} onClick={() => !isAlreadyAdded && toggleUserSelection(user.id)} className={`flex items-center justify-between p-3 rounded-md cursor-pointer ${isAlreadyAdded ? 'bg-gray-100 text-gray-500' : isSelected ? 'bg-blue-50 border border-blue-200' : 'hover:bg-gray-50 border border-gray-100'}`}>
                        <div className="flex flex-col">
                          <span className="font-medium">{user.rank} {user.warName}</span>
                          <span className="text-xs text-gray-500">{user.email}</span>
                        </div>
                        <div>
                          {isAlreadyAdded ? <span className="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded">Já adicionado</span> : isSelected ? <Check className="h-5 w-5 text-blue-500" /> : null}
                        </div>
                      </div>;
            })}
        ...
        <DialogFooter className="flex justify-between sm:justify-between">
          ...
          <Button onClick={handleAddVolunteers} disabled={selectedUsers.length === 0}>
